...
import SpecialOffer from '../components2/Specialoffer'
...
const Banner = React.memo(({products}) => {
    ...
    return (
        <div
            className="bg-white max-h-screen w-auto rounded-lg shadow-md mb-6 flex flex-col items-center justify-center overflow-hidden cursor-pointer"
            onClick={() => {
              router.push(`/product/${products[current]._id}`);
            }}
        >
           <div className="w-auto h-[calc(100vh-150px)]">
           <img
                src={products[current]?.productImage[0]}
                alt={products[current]?.itemName}
                className="h-[calc(100vh-200px)]  object-cover cursor-pointer"
                onClick={() => {
                    router.push(`/product/${products[current]._id}`);
                }}
            />
    ...
    <div className="w-full bg-gray-100 min-h-screen ">
    {
        isLoading ?
            ...
            <main className="p-4">
                ...
                <div className="flex space-x-2 mb-6 overflow-x-auto mt-12 pb-2">
                    {visibleTabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`px-4 py-2 rounded-full whitespace-nowrap ${activeTab === tab.id
                        ...
                        >
                            ...
                            <div className="p-2">
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    <div className="flex ">
                                    <span className="text-grey opacity-40 pr-2 line-through font-bold text-sm">
                                        Rs {product.mrp}/-
                                    </span>
                                ...
                                <div className="flex justify-between m-2 flex-row mt-2">
                                    <div className="flex space-x-2 mt-2 flex-row">
                                        <Heart className="text-red-500 cursor-pointer" size={20} /> 
                                        <MessageSquare className="text-gray-400" size={20} />
                                        <ShoppingCart className="text-gray-400" size={20} />
                                    </div>
                                    <div className="flex space-x-2  flex-row p-2 bg-blue-600">
                                        <button className=" text-white font-bold text-sm">Buy Now</button>
                                    </div>
